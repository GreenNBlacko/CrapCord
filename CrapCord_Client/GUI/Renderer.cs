...
public class Renderer {
    ...
    protected void Render() {
        ...
        ImGui.Begin($"CrapCord v{ctx.config["CLIENT_VERSION"]}", flags);
        ...
        private byte[] LoadEmbeddedShaderCode(ResourceFactory factory, string name, ShaderStages stage) {
            switch (factory.BackendType) {
                case GraphicsBackend.Direct3D11: {
                    string resourceName = "CrapCord_Client.Shaders.HLSL." + name + ".hlsl.bytes";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.OpenGL: {
                    string resourceName = "CrapCord_Client.Shaders.GLSL." + name + ".glsl";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.Vulkan: {
                    string resourceName = "CrapCord_Client.Shaders.SPIR-V." + name + ".spv";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                case GraphicsBackend.Metal: {
                    string resourceName = "CrapCord_Client.Shaders.Metal." + name + ".metallib";
                    return GetEmbeddedResourceBytes(resourceName);
                }
                default:
